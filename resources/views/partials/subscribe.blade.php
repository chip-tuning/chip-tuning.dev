<section class="section dark-bg pv-40 clearfix">
	<div class="container">
		<div class="row">
			<div class="col-md-12">
				<div class="call-to-action text-center">
					<div class="row">
						<div class="col-sm-8 col-sm-offset-2">
							<h2 class="title">Newsletter</h2>
							<p>Ako vam se dopadaju članci na našem blogu, upišite svoju email adresu i tako budite u toku sa svim novostima.</p>
							<div class="separator"></div>
							<form id="subscribe-form" class="form-inline has-feedback">
								<div class="form-group has-feedback">
									<label class="sr-only" for="email">Email adresa</label>
									<input id="email" class="form-control has-feedback" name="email" type="email" placeholder="Unesite vašu e-mail adresu">
									<i class="fa fa-envelope form-control-feedback"></i>
								</div>
								<button class="btn btn-gray-transparent btn-animated margin-clear" type="submit">Prijavi se <i class="fa fa-send"></i></button>
							</form>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>